...
| 素手：両手で親指と中指のピンチ | リセンター（両スティック同時クリックに相当） |
...
手先を保持します（コントローラのトラッキングロスと同じ扱い）。**絶対マッピングでは、つまんだ瞬間から
手先が手の位置へ寄っていく**のはコントローラと同じです。`--hand-clutch always` は手が見えた瞬間に
動き出すので、実機ではまず相対マッピングか手を置いた状態で試してください。
